    ...
    fetch('http://127.0.0.1:8000/api/campaigns_and_teams')
      ...
      .then(data => {
        ...
        const campaigns = filtered.map(item => ({
          name: item.team && item.team.team_name ? item.team.team_name : "No Team",
      ...
      <div className='flex'>
        ...
        <div className='w-full mt-8 mr-5 md:ml-12'>
          ...
          <div className='w-full flex-col flex gap-[32px]'>
            <div className='flex flex-col w-full gap-10 p-5 pb-12 mt-3 card' id="#currentCampaigns" ref={currentCampaignRef}>
    ...
    currentCampaigns.map((campaign, index) => (
      <div className='flex flex-col items-center gap-4' key={index}>
        ...
        <p className='w-[124px] h-[35px] flex items-center justify-center text-[14px] leading-[25px] font-[400] rounded-[192px] bg-themeGreen text-white text-center'>{campaign.name}</p>
            ...
            <Campaign_table/>
            <Add_New_Campaign className="mt-60" set={isCreated} setter={setIsCreated} ref={addNewCampaignRef} />
          </div>
        </div>
      </div>
    </>
