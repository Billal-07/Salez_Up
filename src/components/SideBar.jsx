...
import { Campaigns, Commission, Dashboard, Help, Prizes, SaleAgents, SetContest, TVScreen, TeamLeaders, Teams } from '../../public/icons/Icons';
import { FaArrowRight } from 'react-icons/fa';
import { useScrollContext } from '../contexts/scrollContext';

const SideBar = ({ name = "none" }) => {

    const { currentCampaignRef,
        addNewCampaignRef,
        currentTeamsRef,
        addNewTeamRef,
        orgChartRef,
        teamLeaderRef,
        addNewTeamLeaderRef } = useScrollContext();

    const links = [
        {
            name: 'Dashboard',
            icon: <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M10.687 6.41221V0H19.2366V6.41221H10.687ZM0 10.687V0H8.54962V10.687H0ZM10.687 19.2366V8.54962H19.2366V19.2366H10.687ZM0 19.2366V12.8244H8.54962V19.2366H0Z" fill="#626E70" />
            </svg>,
            link: "/dashboard",
        },
        {
        ...
        },
        {
            ...
            icon: <svg width="20" height="18" viewBox="0 0 20 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M7.51178 7.07824C7.91088 7.07824 8.30608 6.99964 8.6748 6.8469C9.04352 6.69417 9.37855 6.47031 9.66076 6.18811C9.94297 5.9059 10.1668 5.57087 10.3196 5.20214C10.4723 4.83342 10.5509 4.43823 10.5509 4.03912C10.5509 3.64002 10.4723 3.24482 10.3196 2.8761C10.1668 2.50738 9.94297 2.17235 9.66076 1.89014C9.37855 1.60793 9.04352 1.38407 8.6748 1.23134C8.30608 1.07861 7.91088 1 7.51178 1C6.70575 1 5.93274 1.32019 5.36279 1.89014C4.79285 2.46008 4.47266 3.2331 4.47266 4.03912C4.47266 4.84515 4.79285 5.61816 5.36279 6.18811C5.93274 6.75805 6.70575 7.07824 7.51178 7.07824Z" fill="#626E70" stroke="#626E70" stroke-width="1.73664" stroke-linecap="round" stroke-linejoin="round" />
                <path d="M13.1562 7.07835L15.7612 4.47339L18.3662 7.07835" stroke="#626E70" stroke-width="1.73664" stroke-linecap="round" stroke-linejoin="round" />
                <path d="M1 16.1086V16.6296H14.0248V16.1086C14.0248 14.1636 14.0248 13.1911 13.6462 12.4478C13.3132 11.7943 12.7819 11.263 12.1284 10.93C11.3851 10.5514 10.4126 10.5514 8.46756 10.5514H6.55725C4.61221 10.5514 3.63969 10.5514 2.89641 10.93C2.2429 11.263 1.71158 11.7943 1.37859 12.4478C1 13.1911 1 14.1636 1 16.1086Z" fill="#626E70" stroke="#626E70" stroke-width="1.73664" stroke-linecap="round" stroke-linejoin="round" />
            </svg>
            ,
            link: "/teamLeader/#orgChart",
            children: [
                // { name: "ORG Chart", link: "/teamLeader/#orgChart", ref: orgChartRef },
                { name: "Team Leader", link: "/teamLeader/#currentTeamLeaders", ref: teamLeaderRef },
                { name: "Add New Team Leader", link: "/teamLeader/#addNewTeamLeader", ref: addNewTeamLeaderRef },
            ]
        },
...

        {
            ...
            // link: "",
            children: [
                { name: "Head Of Department", link: "/HeadOfDepartment/#CurrentHeadOfDepartment", ref: teamLeaderRef },
                { name: "Add New Head Of Department", link: "/HeadOfDepartment/#AddHeadOfDepartment", ref: addNewTeamLeaderRef },
            ]
        },

        {
            ...
            ,
            children: [
                { name: "Junior Head Of Department", link: "/JuniorHeadOfDepartment/#CurrentJuniorHeadOfDepartment", ref: teamLeaderRef },
                { name: "Add New Junior Head Of Department", link: "/JuniorHeadOfDepartment/#AddJuniorHeadOfDepartment", ref: addNewTeamLeaderRef },
            ]
        },
        {
            type: 'header',
    ...
    if (!isSidebarOpen) return (
        <div className="fixed z-50 flex items-center p-3 my-3 text-2xl text-white cursor-pointer bg-themeGreen w-fit rounded-tr-md rounded-br-md" onClick={() => setIsSidebarOpen((prev) => !prev)}>
    ...
    return (
        ...
        //                                     } else {
        //                                         // Toggle expansion for other links
        //                                         setIsLinkExpanded({ ...isLinkExpanded, [link.name]: !isLinkExpanded[link.name] });
        //                                     }
        //                                 }}
        //                             >
        //                                 <div className={`w-[20px] h-[20px] default-icon group-hover:hovered-icon ${isLinkExpanded[link.name] ? "hovered-icon" : ""}`}>{link.icon}</div>
        //                                 <div className={`text-[14px] font-[500] tracking-wide flex flex-row items-center justify-between w-full ${isLinkExpanded[link.name] ? "text-[#269F8B]" : ""}`}>
        //                                     {link.name}
        //                                     {/* Conditionally render dropdown icon for other links */}
        //                                     {link.children && (
        //                                         <img src="/icons/selectDropdown.png" className={`inline w-[20px] h-[20px] ${isLinkExpanded[link.name] ? "rotate-180" : ""}`} alt="" />
        //                                     )}
        //                                 </div>
        //                             </div>
        //                             {link.children && (
        //                                 <div className={`${isLinkExpanded[link.name] ? "py-[10px] pl-[5px] border-t-[0.8px] border-t-[#3333334D]" : ""}`}>
        //                                     {link.children.map((sublink, subIndex) => (
        //                                         <div key={subIndex} className={`${isLinkExpanded[link.name] ? "block" : "hidden"} hover:text-[#269F8B]`}
        //                                             onClick={() => { setActiveLink(sublink.name, sublink.ref) }}
        //                                         >
        //                                             <NavLink
        //                                                 to={sublink.link}
        //                                                 className={`
        //                                 flex cursor-pointer font-semibold gap-[10px] my-[6px] flex-row group items-center 
        //                                 ${linkState[sublink.name.toLowerCase().replace(/\s+/g, '')] ? "text-[#269F8B]" : "text-themeGray"}
        //                             `}
        //                                             >
        //                                                 <Dot
        //                                                     className={`
        //                                     group-hover:text-[#269F8B] w-[28px] h-[28px] hover:text-[#269F8B]} 
        //                                     ${linkState[sublink.name.toLowerCase().replace(/\s+/g, '')] ? "text-[#269F8B]" : "text-[#D9D9D9]"}
        //                                 `}
        //                                                     strokeWidth={5}
        //                                                 />
        //                                                 <span className="text-[16px] tracking-wide font-[500]">{sublink.name}</span>
        //                                             </NavLink>
        //                                         </div>
        //                                     ))}
        //                                 </div>
...
//     // Initialize states for all menu items
//     useEffect(() => {
...
//                         initialLinkState[child.name.toLowerCase().replace(/\s+/g, '')] = false;
//                     });
//                 }
//             }
//         });
...
//         setIsLinkExpanded(initialExpandedState);
//     }, []);

//     // Handle window resize
//     useEffect(() => {
//         const handleResize = () => {
//             setIsSidebarOpen(window.innerWidth >= 480);
//         };
...
//         const newExpandedState = { ...isLinkExpanded };

//         // Reset all states
//         Object.keys(newLinkState).forEach(key => {
//             newLinkState[key] = false;
//         });
...
//                 if (item.children) {
//                     item.children.forEach(child => {
//                         if (location.pathname.includes(child.link?.toLowerCase() || '')) {
//                             newExpandedState[item.name] = true;
//                             newLinkState[child.name.toLowerCase().replace(/\s+/g, '')] = true;
//                         }
//                     });
//                 }
//             }
...
//         if (fragment) {
//             const fragmentKey = fragment.toLowerCase().replace(/\s+/g, '');
//             newLinkState[fragmentKey] = true;
//         }

//         setLinkState(newLinkState);
//         setIsLinkExpanded(newExpandedState);
//     }, [location]);
...
//         const isExpanded = isLinkExpanded[item.name];
//         const isActive = linkState[item.name.toLowerCase().replace(/\s+/g, '')];

//         return (
//             <div key={item.name} className="border-b-[0.8px] border-b-[#3333334D]">
//                 <div
//                     className={`
//                         flex flex-row items-center cursor-pointer
//                         h-[56px] px-4 gap-[14px] 
//                         text-themeGray hover:text-[#269F8B]
//                         ${isActive ? 'text-[#269F8B]' : ''}
//                     `}
//                     onClick={() => handleMenuClick(item)}
//                 >
//                     <div className={`w-[20px] h-[20px] ${isExpanded ? "text-[#269F8B]" : ""}`}>
//                         {item.icon}
//                     </div>
...
//                                 className={`w-[20px] h-[20px] transition-transform ${isExpanded ? "rotate-180" : ""}`}
//                                 alt=""
//                             />
//                         )}
//                     </div>
...
//                                     text-themeGray hover:text-[#269F8B]
//                                     ${linkState[child.name.toLowerCase().replace(/\s+/g, '')] ? 'text-[#269F8B]' : ''}
//                                 `}
//                                 onClick={() => setActiveLink(child.name, child.ref)}
//                             >
//                                 <Dot className="w-[20px] h-[20px]" strokeWidth={3} />
//                                 <span className="text-[14px] font-[500]">{child.name}</span>
//                             </NavLink>
//                         ))}
//                     </div>
//                 )}
